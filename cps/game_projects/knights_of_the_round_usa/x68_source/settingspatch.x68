...
tryUnlockInput:
        ...
        cmpi.w #$ffff, $ff0008 ;if everything is set, there are no inputs being held
        bne checkLocked
...
dip_B7: ;dip: 1000 0000 - P2 Up
        ...
        bne dip_B6
        eori.b #$80, $ff0006 ;Player Type: 2p or 3p
        move.b #$ff, $ff0004 ;lock inputs

dip_B6: ;dip: 0100 0000 - P2 Right
        cmpi.l #$feffff00, $ff0008
        bne dip_B5
        eori.b #$40, $ff0006 ;Coin shooter
        move.b #$ff, $ff0004 ;lock inputs

dip_B5: ;dip 0010 0000 - P2 Down
        cmpi.l #$fbffff00, $ff0008
        bne dip_B4
        eori.b #$20, $ff0006 ; Level 2: Normal / Difficult (works with B4,B3)
        move.b #$ff, $ff0004 ;lock inputs

dip_B4: ;dip: 0001 0000 - P2 Left
        cmpi.l #$fdffff00, $ff0008
        bne dip_B3
        eori.b #$10, $ff0006 ;Level 2: Normal / Difficult (works with B5,B3)
        move.b #$ff, $ff0004 ;lock inputs
        
dip_B3: ;dip: 0000 1000 - P2 Up + P1 Start
        cmpi.l #$f7ffef00, $ff0008
        bne dip_B2
        eori.b #$08, $ff0006 ;Level 2: Normal / Difficult (works with B5,B4)
        move.b #$ff, $ff0004 ;lock inputs
        
dip_B2: ;dip: 0000 0100 - P2 Right + P1 Start
        cmpi.l #$feffef00, $ff0008
        bne dip_B1
        eori.b #$04, $ff0006 ;Level 1: Normal / Difficult (works with B1,B0)
        move.b #$ff, $ff0004 ;lock inputs
        
dip_B1: ;dip: 0000 0010 - P2 Down + P1 Start
        cmpi.l #$fbffef00, $ff0008
        bne dip_B0
        eori.b #$02, $ff0006 ;Level 1: Normal / Difficult (works with B2,B0)
        move.b #$ff, $ff0004 ;lock inputs

dip_B0: ;dip: 0000 0001 - P2 Left + P1 Start
        cmpi.l #$fdffef00, $ff0008
        bne dip_C7
        eori.b #$01, $ff0006 ;Level 1: Normal / Difficult (works with B2,B1)
        move.b #$ff, $ff0004 ;lock inputs

;START DIP C
dip_C7: ;dip: 1000 0000 - P1 Up + P2 Start
        cmpi.l #$fff7df00, $ff0008
        bne dip_C6
        eori.b #$80, $ff0007 ;???
        move.b #$ff, $ff0004 ;lock inputs
...
dip_C6: ;dip: 0100 0000 - P1 Right + P2 Start
        ...
        bne dip_C5
        eori.b #$40, $ff0007 ;Continues On / Off
        move.b #$ff, $ff0004 ;lock inputs

dip_C5: ;dip 0010 0000 - P1 Down + P2 Start
        cmpi.l #$fffbdf00, $ff0008
        bne dip_C4
        eori.b #$20, $ff0007 ;Demo Sound On / Off
        move.b #$ff, $ff0004 ;lock inputs

dip_C4: ;dip: 0001 0000 - P1 Left + P2 Start
        cmpi.l #$fffddf00, $ff0008
        bne dip_C3
        eori.b #$10, $ff0007 ;Flip Screen
        move.b #$ff, $ff0004 ;lock inputs
        
dip_C3: ;dip: 0000 1000 - P2 Up + P2 Start
        cmpi.l #$f7ffdf00, $ff0008
        bne dip_C2
        eori.b #$08, $ff0007 ;FREEZE GAME - CAUTION this will basically soft lock the game
        move.b #$ff, $ff0004 ;lock inputs
        
dip_C2: ;dip: 0000 0100 - P2 Right + P2 Start
        cmpi.l #$feffdf00, $ff0008
        bne dip_C1
        eori.b #$04, $ff0007 ;Free Play On / Off
        move.b #$ff, $ff0004 ;lock inputs
...
;chunk
    ...
    org $F7450 ;Write service menu val to D2
        ...
        bne exit
        move.b #$BF, D2 ;start buttons are being held, so write that to D2 - JERED CHANGE THIS TO JUST LITERALLY SET THE SERVICE MENU BIT, which is BF
exit:
        jmp $CF8
;chunk

    org $684b6 ;Called when setting the index for the service submenu to enter.
        jmp $F7500
;chunk
...
;chunk
    ...
    ;Note: The instruction we care about is actually at 686cc, but its only 4 bytes
...
;chunk
    ...
    org $f7550
        beq beqTo686d4 ;original instruction: 686c8 BEQ $686d2 - since we cant BEQ that far in memory, we beq here and then jmp.
